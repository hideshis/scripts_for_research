...
def edit_dependencyManagemt(dependencyManagement):
    dependencyManagement_children_list = [dependencyManagement_child.tag.split('}')[-1] for dependencyManagement_child in list(dependencyManagement) if True]
    if 'dependencies' not in dependencyManagement_children_list:
        dependencies_tag = 'dependencies'
        new_tag_append(dependencyManagement, dependencies_tag, '', {})
    else:
        for dependencyManagement_child in list(dependencyManagement):
            if dependencyManagement_child.tag.endswith('dependencies'):
                dependencies_tag = dependencyManagement_child.tag
                break
    dependencies = dependencyManagement.find(dependencies_tag)
    new_tag_insert(dependencies, 0, 'dependency', '', {})
    new_tag_append(dependencies, 'dependency', '', {})
    dependency1 = dependencies.findall('dependency')[0]
...
if root.find(properties_tag) == None:
    new_tag_insert(root, insert_location, properties_tag, '', {})
    insert_location += 1
edit_properties(root.find(properties_tag))
