...
header = ['project', 'num_covered_instruction', 'num_uncovered_instruction', 'C0coverage']
csvWriter.writerow(header)
f_read = open('coverage_info.csv', 'r')
pjt_id = f_read.readline().split(',')[0]
coverage_of_pjt = f_read.readline().split(',')[-1].replace('\r','').replace('\n','')
	...
	if coverage_report_existence_flag == True:
		...
		for html_path in result_list:
			...
			for tr_counter in range(1, len(root.xpath("/html/body/table/tbody/tr"))+1):
				C0coverage = root.xpath("/html/body/table/tbody/tr[" + str(tr_counter) + "]/td[3]")[0].text
				print C0coverage
				if C0coverage == '0%':
					...
					for img_counter in range(1,3):
						try:
							...
							num_covered_instruction = 'N/A'
							num_uncovered_instruction = 'N/A'
					print num_covered_instruction, num_uncovered_instruction
				print num_covered_instruction, num_uncovered_instruction
				coverage_info = []
