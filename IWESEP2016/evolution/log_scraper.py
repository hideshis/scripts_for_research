...
the log file is extracted by the following command:
	git log --pretty=format:"commit:%H%nauthor:%an%ndate:%at" --name-only > log_target.txt
...
def csv_writer_tc(pjt_name, commit_hash, date, author, files):
	f = open(pjt_name + '_tc.csv', 'a')
	csvWriter = csv.writer(f, lineterminator="\n")
	size = os.path.getsize(pjt_name + '.csv')
	if size == 0:
		title = ["Commit_hash", "Date", "Author", "File"]
...
def csv_writer(pjt_name, commit_hash, date, author, files):
	...
	size = os.path.getsize(pjt_name + '.csv')
	if size == 0:
		title = ["Commit_hash", "Date", "Author", "File", "Attribution"]
	...
	while line:
		...
		while (not line == "\n") and (line):
			...
			if "/test/" in file_name and source_code_name.endswith(".java"):
				...
				os.system('echo ' + file_name + '>>pc_list.txt')
			else:
				line = f.readline()
				continue
			file_info = []
		...
		if len(file_list) > 0:
			...
			for file_info in file_list:
				if file_info[1] == "test":
					csv_writer_tc(pjt_name, commit_hash, date, author, file_list)
					tc_count += 1
					break
		line = f.readline()
	f.close()
...
if __name__ == "__main__":
	...
	os.system("cat pc_list.txt | sort | uniq > hoge.txt")
	os.system("mv hoge.txt pc_list.txt")
	os.system("cat tc_list.txt | sort | uniq > hoge.txt")
	os.system("mv hoge.txt tc_list.txt")
