...
def num_bug_getter(tc_name, linked_pc, tc_born_time, tc_dead_time):
    try:
        ...
        for x in result_list:
            ...
            if (tc_dead_time >= bug_introducing_time) and (bug_introducing_time >= tc_born_time):
                num_bug += 1
                os.system("echo " + tc_name + "," + x.split(",")[1] + "," + x.split(",")[0] + ">>bug_info_detail.csv")
        return num_bug
...
def ave_bug_calcurator(tc_name, lifetime, tc_born_time, tc_dead_time):
    ...
    for linked_pc in linked_pc_list:
        num_bug = num_bug_getter(tc_name, linked_pc, tc_born_time, tc_dead_time)
